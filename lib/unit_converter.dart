...
class _UnitConverterState extends State<UnitConverter> {
  ...
  void _updateConversion() async {
    ...
    if (widget.category.name=='Currency') {
      var _convertedValuez = await Api.convert('currency',_inputValue.toString(),
          _toValue.name,_fromValue.name);
      if (_convertedValuez==null)
        setState(() {
          _showErrorUI = true;
        });

      result = _format(_convertedValuez);
  ...
  Widget build(BuildContext context) {
    if ((widget.category.units==null) || (
        (widget.category.name=='Currency')&&(_showErrorUI)
    ))
      return
          Container(
            decoration: BoxDecoration(
          ...
          child: Column(
            children: <Widget>[
              Icon(Icons.error_outline, size: 180, color: Colors.white),
